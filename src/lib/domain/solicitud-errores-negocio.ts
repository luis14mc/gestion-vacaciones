...
const PATRONES_VALIDACION_NEGOCIO = [
  ...
  /^Debe solicitar con al menos \d+ día\(s\) de anticipación\. Fecha mínima permitida: .+\.$/,
  /^Las vacaciones deben solicitarse con al menos \d+ día\(s\) de anticipación\.$/,
  /^No puede solicitar permisos para fechas anteriores a la fecha actual\.$/,
  /^La fecha fin no puede ser anterior a la fecha inicio\.$/,
  /^No se pueden solicitar permisos para sábado o domingo\.$/,
  /^La hora de regreso debe ser posterior a la hora de salida\.$/,
  /^El permiso 1–2 horas no puede exceder 2 horas\.$/,
  /^El permiso de medio día no puede exceder 4 horas\.$/,
  ...
  /^No tiene días disponibles suficientes\.$/,
